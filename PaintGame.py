...
def drawCardStackLocation():
    stackLocationX,stackLocationY = 440, 30
    offset = 190
...
def drawTableau():
    cardlocationx,cardlocationy = -180,350
    for i in range(8):
        for f in range(i):
            pygame.draw.rect(Assets.WIN, Assets.BLACK, (cardlocationx+4+(250*i),cardlocationy+4+(50*f), Assets.CARDWIDTH,Assets.CARDHEIGHT),width=0, border_radius=13)
            pygame.draw.rect(Assets.WIN, Assets.WHITE, (cardlocationx+(250*i),cardlocationy+(50*f), Assets.CARDWIDTH,Assets.CARDHEIGHT),width=0, border_radius=13)
            if(f < i):
                Assets.WIN.blit(Assets.cardback_black, (cardlocationx+9+(i*250),cardlocationy+11+(f*50)))
